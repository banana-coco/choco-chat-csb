...
### CodeSandbox
1. CodeSandboxでこのリポジトリをインポート
2. 自動的に依存関係がインストールされる
3. ポート5000でサーバーが起動

設定ファイル:
- `sandbox.config.json` - レガシーサンドボックス用
- `.codesandbox/tasks.json` - VMサンドボックス用

## データベース設定

### 設定値
データベース接続情報は `db.js` に直接設定されています。
...
### ストレージの動作
- PostgreSQL使用（メッセージ永久保存）
- データベース接続に失敗した場合 → エラーメッセージ表示（原因と解決方法を表示）
